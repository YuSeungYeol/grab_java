...
public class HospiInfoDao {
	...
	public List<HospiInfo> selectHospiInfoList(HospiInfo option, Connection conn){
		...
		try {
			String sql = "SELECT * FROM hospital WHERE hospital_whether NOT IN ('Y', 'y')";
			if(option.getHospital_name() != null) {
				sql += " AND hospital_name LIKE CONCAT('%',?,'%')";
			}
	...
	public HospiInfo viewHospiInfo(int hospital_no, Connection conn) {
		...
		try {
			String sql = "SELECT `hospital_no`, `hospital_name`, `hospital_phone`, `hospital_addr`, `reg_date` "
			...
			rs = pstmt.executeQuery();
			 ...
			 if(rs.next()) { 
				 ...
				 hi.setHospital_reg_date(rs.getTimestamp("reg_date").toLocalDateTime());
