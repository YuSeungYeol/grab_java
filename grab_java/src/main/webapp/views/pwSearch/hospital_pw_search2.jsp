...
<body>
    ...
    <div class="container">
        ...
        <div>
            <p><strong>비밀번호를 변경해 주세요. 이전에 썼던 비밀번호를 제외하고 입력해 주세요.</strong></p>
        </div>
        <div class="signup-content">
            <div class="form-group">
                <label for="username">아이디</label>
               <span id="hospital-name"><%= request.getParameter("member_name") %></span>
                <input type="hidden" name="name" value="<%= request.getParameter("member_name") %>">
                <input type="hidden" name="email" value="<%= request.getParameter("hospital_email") %>">
            </div>
            <div class="form-group">
            ...
            <button type="button" class="signup-button" onclick="checkAgreement">확인</button>
        </div>
        <div class="description-box">
            <p>• 영문 대/소문자, 숫자, 특수문자가 꼭 포함되어야 합니다.</p>
            <p>• 비밀번호는 최소 8자리, 최대 16자리로 지정해주시기 바랍니다.</p>
    ...
    function validateAndSubmit() {
        ...
        if (newPassword.length < 8 || newPassword.length > 16) {
            alert('비밀번호는 8자리 이상, 16자리 이하로 설정해야 합니다.');
            return false;
        }
