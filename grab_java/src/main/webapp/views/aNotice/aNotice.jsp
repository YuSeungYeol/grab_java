                ...
                <div class="card">
                  <div class="card-body">
                    <table class="table table-bordered">
                      ...
                      <tbody>
						<%
							List<Notice> list = (List<Notice>)request.getAttribute("resultList");
						if(list != null){
							for(int i = 0 ; i < list.size(); i++){ %>
							 	
								<tr>
									<td><a href="<%=request.getContextPath()%>/member/toNoticeView?notice_no=<%= list.get(i).getNotice_no() %>"
                    ...
                    <div class="btnBox">
                      <a href="<%=request.getContextPath()%>/member/toNoticeNew"><button class="btn btn-warning"><b>작성하기</b></button></a>
                    </div>
                    <br>
                   <% Notice paging = (Notice)request.getAttribute("paging");%>
					<% if(paging != null){ %>
						<div class="center">
							<div class="pagination">
								<%if(paging.isPrev()){%>
									<a href="<%=request.getContextPath()%>/member/toNoticeView?nowPage=<%=(paging.getPageBarStart()-1)%>">&laquo;</a>
								<%}%>
								<% for(int i=paging.getPageBarStart(); i<= paging.getPageBarEnd(); i++){ %>
									<a href="<%=request.getContextPath()%>/member/toNotice?nowPage=<%=i%>"
