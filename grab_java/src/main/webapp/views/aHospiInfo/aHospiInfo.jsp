                  ...
                  <div class="card-body">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                       	...
                       	<%
							List<HospiInfo> list = (List<HospiInfo>)request.getAttribute("resultList");
						if(list != null){
							for(int i = 0 ; i < list.size(); i++){ %>
								<tr>
									<td><a href="<%=request.getContextPath()%>/member/infoCheck?hospital_no=<%= list.get(i).getHospital_no()%>">
											<%=list.get(i).getHospital_name() %>
										</a>
									</td>
									
									<%
									    LocalDateTime regDateTime = list.get(i).getHospital_reg_date();
									    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
									    String formattedDate = regDateTime.format(formatter);
									%>
									<td><%= formattedDate %></td>
									<td><%=list.get(i).getHospital_org_license() %></td>
									<td><%=list.get(i).getHospital_org_registration() %></td>
									<%if(list.get(i).getHospital_whether() == 'x' ||list.get(i).getHospital_whether() == 'X'){ %>
										<td><b>반려</b></td>
									<%} else if(list.get(i).getHospital_whether() == 'n' ||list.get(i).getHospital_whether() == 'N'){ %>
										<td><b>미확인</b></td>
									<%} %>
								</tr>
								<%}%>
	                  ...
	                  <br>
                   <% HospiInfo paging = (HospiInfo)request.getAttribute("paging");%>
					<% if(paging != null){ %>
						<div class="center">
							<div class="pagination">
								<%if(paging.isPrev()){%>
									<a href="<%=request.getContextPath()%>/member/toHospiInfo?nowPage=<%=(paging.getPageBarStart()-1)%>">&laquo;</a>
								<%}%>
								<% for(int i=paging.getPageBarStart(); i<= paging.getPageBarEnd(); i++){ %>
									<a href="<%=request.getContextPath()%>/member/toHospiInfo?nowPage=<%=i%>"
									<%=paging.getNowPage() == i ? "class='active'" : ""%>>
										<%=i%>
									</a>
								<%}%>
<%-- 								<% if(paging.isNext()){%>
									<a href="<%=request.getContextPath()%>/member/toNotice?nowPage=<%=(paging.getPageBarEnd()+1)%>">&raquo;</a>
								<%}%> --%>
							</div>
						</div>
					<%}%>
