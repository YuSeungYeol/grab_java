...
<head>
    ...
    <link rel="stylesheet" href="../../../resources/member/memberFind/find_id_2.css" type="text/css">
</head>
...
<body>
    <div class="container">
        ...
        <div class="id-show">
            <ul>
                <li>아이디 : <span id="member-id"></span></li>
            </ul>
        </div>

        <div class="id-button">
            <div class="login-go">
                <a href="<%=request.getContextPath()%>/views/member/memberLogin/login.jsp">로그인하기</a>
            </div>
            <div class="find-pw">
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response && response.memberId) {
                        $('#member-id').text(response.memberId);
                    } else {
                        $('#member-id').text('일치하는 아이디가 없습니다.');
                    }
                },
                error: function (error) {
                    console.error('Error fetching member ID:', error);
                    $('#member-id').text('아이디를 가져오는 중 오류가 발생했습니다.');
                }
            });
        });
    </script>
